...
import { animate, type HTMLMotionProps, motion, useInView, useMotionValue, useReducedMotion, useTransform } from "motion/react";
...
function measureWidths(el: HTMLElement, texts: string[]) {
   ...
   const widths = texts.map(t => {
      ghost.textContent = t;
      return Math.ceil(ghost.getBoundingClientRect().width) + 4;
   });
...
export function DiaTextReveal({
   ...
   return (
      <motion.span
         ref={spanRef}
         className={cn("align-bottom leading-[0.5] pr-1 text-inherit", className)}
         style={{
            backgroundClip: "text",
            backgroundImage,
